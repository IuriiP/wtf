    ...
    static public function show($arr, $templates, $marker = null)
    {
        ...
        foreach ($arr as $key => $item) {
            if (is_scalar($item)) {
                ...
                $branch = implode(PHP_EOL, self::show($item, [$scalar,
                            $complex], empty($marker[$key]) ? $marker : $marker[$key]));
                if ($complex) {
                    $arr[$key] = sprintf($complex, $key, $branch, empty($marker[$key]) ? '' : $marker[$key]);
                }
            }
        }
        return $arr;
